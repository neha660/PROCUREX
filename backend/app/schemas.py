...
class BudgetPool(BaseModel):
    ...
    def remaining_inr(self) -> float:
        # allocated_inr already reflects post-negotiation spend, so saved_inr
        # is unspent money that's still part of (total - allocated) — adding
        # it again here would double-count it.
        return self.total_inr - self.allocated_inr
